...
async def force_terminate_task_group():
    ...
    while True:
        print('force_terminate_task_group')
        if cancelled:
            raise TerminateTaskGroup()
        await asyncio.sleep(0.5)

async def prepare_weather_data():
    global weather, writers, cancelled
    try:
        while True:
...
async def main():
    try:
        async with asyncio.TaskGroup() as group:
            group.create_task(run_server(), name='server')
            group.create_task(prepare_weather_data(), name='prepare_weather_data')
